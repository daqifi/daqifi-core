using Daqifi.Core.Device.Network;

namespace Daqifi.Core.Tests.Communication.Messages;

public class DaqifiOutMessageTests
{
    [Fact]
    public void SystemInfoResponse_ValidInput_ParsesCorrectly()
    {
        // Captured from Wireshark from a real DAQiFi Nyquist device (SYSTem:SYSInfoPB? response)
        var systemInfoResponse = new byte[]
        {
            
        };

        Stream stream = new MemoryStream(systemInfoResponse);
        var protobufMessage = DaqifiOutMessage.Parser.ParseDelimitedFrom(stream);

        Assert.Equal("NYQUIST", protobufMessage.HostName);
        Assert.Equal((uint)4, protobufMessage.WifiSecurityMode);
        Assert.Equal((uint)9760, protobufMessage.DevicePort);
        Assert.Equal("CenturyLink0624", protobufMessage.Ssid);
        Assert.Equal("192.168.0.45", NetworkAddressHelper.GetIpAddressString(protobufMessage));
        Assert.Equal("Nq1", protobufMessage.DevicePn);
        Assert.Equal("1.0.3", protobufMessage.DeviceFwRev);
    }
}
