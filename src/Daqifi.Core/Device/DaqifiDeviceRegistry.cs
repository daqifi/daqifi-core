...
/// Pruning is only as good as the transport's drop detection, and that now covers a physically
/// unplugged USB device: <see cref="Communication.Transport.SerialStreamTransport"/> polls for its
/// port's continued presence and escalates persistently failing I/O, so an unplugged device stops
/// reporting <c>IsConnected</c> within roughly three seconds and is pruned like any other drop
/// (issue #382). The registry does not subscribe to device status events or reconnect on its own;
/// automatic reconnect is issue #379.
/// </para>
/// <para>
